...
def main():
    ...
    if config.TEST.MODEL_FILE and os.path.isfile(test_model_file):      # model file 있는지 check
    ...
    with torch.no_grad():
        for i, (inputs, targets_2d, weights_2d, targets_3d, meta, input_heatmap) in enumerate(tqdm(test_loader)):
            targets_2d, targets_3d = None, None
        ...
        tb = PrettyTable()
        if 'panoptic' in config.DATASET.TEST_DATASET or 'etri' in config.DATASET.TEST_DATASET:
            mpjpe_threshold = np.arange(10, 155, 10)
    ...
    # import pickle
    # dump_dict = {}
    # dump_dict['preds'] = preds
    # dump_dict['metas'] = metas
    # with open("/home/vis/projects/voxelpose-pytorch/Variables/preds", "wb") as wf:
    #     pickle.dump(dump_dict, wf)
