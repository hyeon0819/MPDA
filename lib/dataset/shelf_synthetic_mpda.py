...
from utils.generate_heatmap import check_occlusion_camera_coco, occlusion_level
...
class ShelfSynthetic_mpda(Dataset):
    def __init__(self, cfg, image_set, is_train, transform=None):
        ...
        self.cam_list = ['0', '1', '2', '3', '4'][:cfg.DATASET.CAMERA_NUM]
        ...
        # self.db_file = osp.join(dataset_root, 'pkl_file/mpda', f'train_mpda_{self.data_num}', f"cam{self.num_views}_aug_ratio_{self.aug_ratio:02d}.pkl")
        self.db_file = f'mpda_{self.image_set}_data{self.data_num:03d}_aug{self.aug_ratio:02d}.pkl'
    ...
    def _get_cam(self):
        ...
        for id, cam in list(cameras.items()):
            if id not in self.cam_list:
                del cameras[id]
                continue
    ...
    def __getitem__(self, idx):
        ...
        for k, cam in self.cameras.items():
            _, th, tw, t3, m, ih = self._get_single_view_item(joints_3d, joints_3d_vis, cam)
            # input.append(i)
    ...
    def _get_single_view_item(self, joints_3d, joints_3d_vis, cam):
        ...
        camera_t = cam['T'].reshape(3)
        joints_occ = check_occlusion_camera_coco(joints_3d, camera_t)
        joints_occ_level = occlusion_level(joints_occ, dataset='shelf')
        if self.heatmap_generation == 'base':
            input_heatmap = self.generate_input_heatmap_base(joints, joints_vis)
        elif self.heatmap_generation == 'target':
            input_heatmap = self.generate_target_heatmap(joints, joints_vis)
    ...
    def generate_input_heatmap_base(self, joints, joints_vis):
        '''
                ...
                for joint_id in range(num_joints):
                    ...
                    # occlusion joint에 대한 처리
                        ...
                        if joints_occ_level is None:
                            ...
                            lower_intensity = 1 - joints_occ_level[n][joint_id, 0] / 340
                        g = g * lower_intensity
                        while True:
                            # (2) random shift
                            if joints_occ_level is None:
                                ...
                                mu_occ = np.random.normal(loc=0.0, scale=joints_occ_level[n][joint_id, 0]/100, size=2)
