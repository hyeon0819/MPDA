...
VAL_LIST = ['170404_haggling_a1', '170915_toddler5', '161029_build1', '170221_haggling_b1']
TEST_LIST = ['160906_pizza1', '160422_haggling1', '160906_ian5', '160906_band4']
...
def get_db_indices(cam_num, indices):
    db_indices = []
    for index in indices:
        for i in range(cam_num):
            db_indices.append(index * cam_num + i)
    return db_indices
...
class Panoptic(JointsDataset):
    def __init__(self, cfg, image_set, is_train, transform=None):
        ...
        elif self.image_set == 'test' or self.image_set == 'test_occ':
            self.sequence_list = TEST_LIST
            self._interval = 12 

        if self.image_set == 'train':
            self.db_file = f'original_{self.image_set}_cam{self.num_views}_data{self.data_num:03d}.pkl'
        else:
            self.db_file = f'original_{self.image_set}_cam{self.num_views}.pkl'
        self.db_file = os.path.join(self.dataset_root, 'pkl_file', 'voxelpose', self.db_file)
    ...
    def _get_db(self):
        ...
        if self.image_set == 'train':
            db_num = len(db)
            selected_db_num = int((db_num / self.num_views) * (self.data_num/100)) * self.num_views
            return db[:selected_db_num]
        elif self.image_set == 'test_occ':
            selected_indices = np.load(f'/home/vis/projects/MPDA/voxelpose-pytorch/run/occlusion_samples/cam{self.num_views}_occ_1.npy')
            db_indices = get_db_indices(self.num_views, selected_indices)
            return np.array(db)[db_indices].tolist()
        elif self.image_set == 'test':
            test_outlier_idx = {103,142,163,549,550,553,732,742,875,876,878,879,885,886,887,888,1054,1062,1063,1064,1065,
                                1080,1081,1084,1088,1091,1092,1205,1208,1209,1355,1362,1366,1588,1589,1692,1693,1694,1695,
                                1696,1697,1698,1699,1700,1701,1702,1703,1747,1748}
            all_indices = set(range(int(len(db)/self.num_views)))
            selected_indices = list(all_indices - test_outlier_idx)
            db_indices = get_db_indices(self.num_views, selected_indices)
            return np.array(db)[db_indices].tolist()
        else:
            return db
