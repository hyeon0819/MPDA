...
# TRAIN_LIST = ['20230703_interaction1',
#               '20230703_pose2',
#               '20230703_walking2',
#               '20230703_calibration1']
# VAL_LIST = ['20230703_walking1']
# TEST_LIST = ['20230703_pose1']
...
class Hospital(JointsDataset):
    def __init__(self, cfg, image_set, is_train, transform=None):
        ...
        if self.num_views == 4:
            self.cam_list = [(0, 1), (0, 2), (0, 3), (0, 4)]
        else:
            self.cam_list = [(0, 0), (0, 1), (0, 2), (0, 3), (0, 4)][:self.num_views]
        ...
        if self.image_set == 'train':
            self.db_file = f'group_{self.image_set}_{self.aug_type}_cam{self.num_views}_{self.cam_sim}_data{self.data_num}.pkl'
        else:
            self.db_file = f'group_{self.image_set}_{self.aug_type}_cam{self.num_views}_{self.cam_sim}.pkl'
        self.db_file = os.path.join(self.dataset_root, 'pkl_file', f'{self.aug_type}', self.db_file)
    ...
    def _get_db(self):
        ...
        if self.image_set == 'train':
            db_num = len(db)
            selected_db_num = int((db_num / self.num_views) * float(self.data_num)) * self.num_views
            return db[:selected_db_num]
        elif self.image_set == 'test_occ':
            selected_indices = np.load(f'/home/vis/projects/MPDA/Faster-VoxelPose/run/occlusion_samples/cam{self.num_views}_occ_10.npy')
