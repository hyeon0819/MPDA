...
from dataset.JointsDataset_mpda import JointsDataset_mpda
from utils.transforms import projectPoints

logger = logging.getLogger(__name__)

TRAIN_LIST = ['train_mpda']
...
class Panoptic_mpda(JointsDataset_mpda):
    def __init__(self, cfg, image_set, is_train, transform=None):
        ...
        if self.image_set == 'train':
            self.sequence_list = [f'train_mpda_{self.data_num:03d}']
            self._interval = 1
        ...
        if self.image_set == 'train':
            self.db_file = f'mpda_{self.image_set}_cam{self.num_views}_data{self.data_num:03d}_aug{self.aug_ratio:02d}.pkl'
    ...
    def _get_db(self):
        ...
        for seq in self.sequence_list:
            ...
            curr_anno = osp.join(self.dataset_root, seq, f"aug_ratio_{self.aug_ratio:02d}", 'hdPose3d_stage1_coco19')
    ...
    def __getitem__(self, idx):
        ...
        for k in range(self.num_views):
            _, t, w, t3, m, ih = super().__getitem__(self.num_views * idx + k)
            # if i is None:
            #     continue
            # input.append(i)
            target.append(t)
