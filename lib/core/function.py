    ...
    with torch.autograd.set_detect_anomaly(True):
        for i, (inputs, targets_2d, weights_2d, targets_3d, meta, input_heatmap) in enumerate(loader):
            ...
            if 'panoptic' in config.DATASET.TEST_DATASET or 'etri' in config.DATASET.TEST_DATASET \
                ...
                pred, heatmaps, grid_centers, loss_2d, loss_3d, loss_cord = model(views=inputs, meta=meta, targets_3d=targets_3d[0],
                                                                                input_heatmaps=input_heatmap)        
            ...
            if i % config.PRINT_FREQ == 0:
                ...
                msg = 'Epoch: [{0}][{1}/{2}]\t' \
                    ...
                    'Memory {memory:.1f}'.format(
                        ...
                        speed=len(targets_2d) * targets_2d[0].size(0) / batch_time.val,
