...
def save_batch_image_with_joints_multi(batch_image,     # image_with_joints
                                 batch_joints,
                                 batch_joints_vis,
                                 num_person,
                                 file_name,
                ...
                if joint[0, 3] >= 0:
                    for k in eval("LIMBS{}".format(len(joint))):
                        ...
                        ax.plot(x, y, z, c=colors[int(n % 10)], lw=1.5, marker='o', markerfacecolor='w', markersize=2,
                                markeredgewidth=1)
        ax.set_xlim([-2000, 2000]);ax.set_ylim([-2000, 2000]);ax.set_zlim([-2000, 2000])
        ax.set_xlabel('X');ax.set_ylabel('Y');ax.set_zlabel('X')
